...
function Auth() {
  ...
  const validationSchema = Yup.object().shape({
    username: Yup.string().required(`${t('yup.userName.required')}`),
    password: Yup.string()
              ...
              <div className="card mb-3">
                <div className="card-body">
                  <div className="pt-4 pb-2">
                    <h5 className="card-title text-center pb-0 fs-4">{t('clientEspaceAccess')}</h5>
                    <p className="text-center small">{t('tapezNomPrenom')}</p>
                  </div>
                  <form className="row g-3 needs-validation" novalidate onSubmit={(e) => {
                    ...
                    <div className="col-12">
                      <label for="username" className="form-label">{t('userName')}</label>
                      <div className="input-group has-validation">
                        ...
                        <div className="invalid-feedback">{t('tapezUserName')}.</div>
                      </div>
                    </div>
                    <div className="col-12">
                      ...
                      <div className="invalid-feedback">{t('tapezPassword')}</div>
                    </div>
                    <div className="col-12">
                      <button className="btn btn-primary w-100" type="submit" disabled={username === '' || password === ''} >{t('Login')}</button>
                    </div>
                    <div className="col-12">
                      vous n'avez pas de compte ? <Link to={'/create-account'}>{t('createFreeAccount')}</Link>
                    </div>
                    <p onClick={handeForgetPasswordClick} className='forgetPasswordlink'>{t('forgetPassword')}</p>
                    {
                      errorMSG !== '' ?
